...
from utils.env_utils import OrderedSet, SetWithCount
...
def generate_tpch_jobs(np_random, timeline, wall_time, actor_id):
    job_dags = OrderedSet()

    stream_interval = args.stream_intervals[actor_id % len(args.stream_intervals)]
...
def generate_jobs(np_random, timeline, wall_time, actor_id=0):
    if args.query_type == 'tpch':
        job_dags = generate_tpch_jobs(np_random, timeline, wall_time, actor_id)
    else:
        raise ValueError('Invalid query type {}'.format(args.query_type))
    return job_dags
