...
class Environment(object):
    ...
    def __init__(self, id):
        ...
        self.moving_delay = args.moving_delay
        self.exec_cap = args.exec_cap
        # isolated random number generator
        self.np_random = np.random.RandomState(seed=args.seed + self._id)
        ...
        for exec_id in range(self.exec_cap):
    ...
    def saturated(self, node):
        # frontier nodes := unsaturated nodes with all parent nodes saturated
        anticipated_task_idx = \
            node.next_task_idx + self.exec_commit.node_commit[node] + self.moving_executors.count(node)
    ...
    def schedule(self):
        ...
        while len(self.exec_commit[source]) > 0 and len(self.exec_to_schedule) > 0:
            ...
            if node is None:
                ...
                if executor.job_dag is not None and any([not n.no_more_tasks for n in executor.job_dag.nodes]):
                ...
                if executor.job_dag == node.job_dag:
                    ...
                    self.timeline.push(self.wall_time.curr_time + self.moving_delay, executor)
                    # keep track of moving executors
                    self.moving_executors.add(executor, node)
    ...
    def step(self, next_node, limit):
        ...
        done = ((len(self.timeline) == 0) or (self.wall_time.curr_time >= self.max_time)) and \
    ...
    def reset(self, max_time=np.inf):
        ...
        self.job_dags = generate_jobs(self.np_random, self.timeline, self.wall_time, self._id)
